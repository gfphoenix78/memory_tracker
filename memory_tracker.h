...
#define HEAP_MEMORY_TRACK_ALLOC(__alloc_func, ctx, size, ...) ({ \
      void *p = (__alloc_func)((ctx), (size), ## __VA_ARGS__); \
      memory_track_alloc((ctx), p, (size), __FILE__, __LINE__); \
      p; \
    })

#define HEAP_MEMORY_TRACK_REALLOC(__realloc_func, ctx, ptr, size) ({ \
      void *new_ptr = (__realloc_func)((ctx), (ptr), (size)); \
      memory_track_realloc((ctx), (ptr), new_ptr, (size), __FILE__, __LINE__); \
      new_ptr; \
    })

#define HEAP_MEMORY_TRACK_FREE(__free_func, ctx, ptr) do{ \
      memory_track_free((ctx), (ptr), __FILE__, __LINE__); \
      (__free_func)((ctx), (ptr)); \
    }while(0)

#define HEAP_MEMORY_TRACK_FREE0(__free_func, ctx, ptr) do{ \
      memory_track_free((ctx), (ptr), __FILE__, __LINE__); \
      (__free_func)((ctx), (ptr)); \
      (ptr) = (void*)0; \
    }while(0)

#define HEAP_MEMORY_TRACK_RESET(__reset_func, ctx) do{ \
      memory_track_reset(ctx, __FILE__, __LINE__); \
      (__reset_func)(ctx); \
    }while(0);

#define HEAP_MEMORY_TRACK_DESTROY(__destroy_func, ctx) do{ \
...
#define HEAP_MEMORY_TRACK_ALLOC(__alloc_func, ctx, size, ...) (__alloc_func)((ctx), (size), ## __VA_ARGS__)
#define HEAP_MEMORY_TRACK_REALLOC(__realloc_func, ctx, ptr, size) (__realloc_func)((ctx), (ptr), (size))
#define HEAP_MEMORY_TRACK_FREE(__free_func, ctx, ptr) (__free_func)((ctx), (ptr))
#define HEAP_MEMORY_TRACK_FREE0(__free_func, ctx, ptr) do{ \
      (__free_func)((ctx), (ptr)); \
      (ptr) = (void*)0; \
    }while(0)
#define HEAP_MEMORY_TRACK_RESET(__reset_func, ctx)       (__reset_func)(ctx)
#define HEAP_MEMORY_TRACK_DESTROY(__destroy_func, ctx)   (__destroy_func)(ctx)
